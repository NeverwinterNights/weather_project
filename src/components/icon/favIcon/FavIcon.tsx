...
import { FavIconPropsType } from './types';

export const FavIcon = React.memo(({ city }: FavIconPropsType) => {
  ...
  const onAddToFavoritesHandler = (): void => {
    if (conditionUtils(favoritesCity, city.cityName, city.country)) {
      return;
    }
  ...
  useEffect(() => {
    const favSearch = favoritesCity.filter(town => town.cityName === city.cityName);
    if (favSearch && favSearch[0]) {
      setActivate(true);
    } else {
      setActivate(false);
    }
